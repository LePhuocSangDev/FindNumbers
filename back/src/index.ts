...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('leave_room', async (gameRoom) => {
    ...
    if (foundRoom) {
      ...
      console.log(`User with ID: ${gameRoom.player} left room: ${gameRoom.roomName}`);
      ...
      if (foundRoom.players.length === 0) {
        // Set a timer to delete the room if no one is in it after 5 minutes
        ...
        // Cancel the timer if another user joins the room
        socket.on('join_room', () => {
          clearTimeout(timer);
        });
      }
  ...
  socket.on('send_message', async (data: { room: string; message: string; author: string }) => {
  ...
  socket.on(
    'send_gameData',
    (data: { chosenNumber: number; room: string; points: number; player: string }) => {
